...
void sendAndRecieve_loop() {
  ...
  Cur.red = (int(number[0]) - '0') * 10 + (int(number[1]) - '0');;
  Cur.blue = (int(number[2]) - '0') * 10 + (int(number[3]) - '0');
  Cur.seq = seq;
...
void updateDisplay(){
  ...
  if (incomingSeq > seq) {
    // red_score = incomingBlue;
    // blue_score = incomingRed;
    red_score = incomingRed;
    blue_score = incomingBlue;
    number[0] = (int(blue_score) / 10) + '0';
    number[1] = (int(blue_score) % 10) + '0';
    number[2] = (int(red_score) / 10) + '0';
    number[3] = (int(red_score) % 10) + '0';
  }
  // delay(1000);
